...
def get_noticias_por_activo(ticker: str, limite: int = 20) -> list[dict]:
    ...
    with _conn() as con:
        row = con.execute(
            "SELECT activo_id, nombre FROM activos_detalles WHERE ticker = ?", [ticker]
        ).fetchone()
        activo_id = row[0] if row else None
        nombre = row[1] if row else None
        ...
        df = con.execute("""
            ...
            FROM noticias_historial nh
            LEFT JOIN noticias_sentimientos ns 
                ON nh.noticia_id = ns.noticia_id 
                AND ns.activo_id = ?
