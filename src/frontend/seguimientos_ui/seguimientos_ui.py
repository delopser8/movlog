...
def render():
    ...
    with col_der:
        ...
        if activos:
            ...
            # Una fila por activo: X | botón-fila invisible sobre HTML
            for i, a in enumerate(activos):
                ...
                # Fila HTML visual
                st.markdown(f"""
                <table class="seg-table" style="margin-top:-1px">
                  <tbody>
                    <tr class="{selected}">
                ...
                # Botones nativos sobre la fila
                cx, csel = st.columns([1, 8])
                with cx:
                    if st.button("✕", key=f"del_{i}", use_container_width=True):
                        ...
                        st.rerun()
                with csel:
                    # botón transparente que ocupa toda la fila visualmente
                    st.markdown(f"""
                    <style>
                    button[data-testid="baseButton-secondary"][kind="secondary"]#sel_{i} {{
                        opacity: 0 !important;
        ...
        # Buscador — isla igual que la card
        st.markdown('<div class="activo-card">', unsafe_allow_html=True)
        ...
        if st.session_state.seg_resultados:
            ...
            for r in st.session_state.seg_resultados:
                if r == "Sin resultados":
                    st.markdown(
                        '<div class="resultado-item" style="color:#4b5563;border-radius:6px">Sin resultados</div>',
                        unsafe_allow_html=True,
                    )
                    continue
